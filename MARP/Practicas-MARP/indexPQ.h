...
template <typename T, typename Elem, typename Comparator = std::less<T>>
class IndexPQ {
public:
...
private:
	...
	// Podría sustituirse por un mapa??
	//std::vector<Elem> posiciones;   // un 0 indica que el elemento no está
	std::map<Elem, int> posiciones;
...
public:
	...
	void push(Elem e, T const& p) {
		if (posiciones.at(e) != std::out_of_range)
	...
	void update(Elem e, T const& p) {
		Elem i = posiciones.at(e);
		if (posiciones.at(e) == std::out_of_range) // el elemento e se inserta por primera vez
			push(e, p);
		else {
			array[i].prioridad = p;
			if (i != Elem() && antes(array[i].prioridad, array[i / 2].prioridad))
				flotar(i);
	...
	void pop() {
		if (size() == 0) throw std::domain_error("No se puede eliminar el primero de una cola vacía.");
			...
			posiciones.at(array[1].elem) = Elem(); // para indicar que no está
			if (size() > 1) {
				array[1] = std::move(array.back());
				posiciones.at(array[1].elem) = 1;
				array.pop_back();
				hundir(1);
			}
			else
...
private:
	...
	void flotar(Elem i) {
		...
		int hueco = i;
		while (hueco != Elem() && antes(parmov.prioridad, array[hueco / 2].prioridad)) {
